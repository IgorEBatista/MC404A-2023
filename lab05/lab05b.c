...
int main()
{
   ...
   //TODO fazer o início do programa
    ...
    int parte = 0, final = 0;
    InstData data;
    int n = read(0,string,30);

    get_inst_data(string, &data);

    switch (data.type) {
    case R:
        final = final | (data.funct7 << 25);
        final = final | (data.rs2 << 20);
        final = final | (data.rs1 << 15);
        final = final | (data.funct3 << 12);
        final = final | (data.rd << 7);
        final = final | (data.opcode << 0);

        break;
    
    case I:
        final = final | (data.imm << 20);
        final = final | (data.rs1 << 15);
        final = final | (data.funct3 << 12);
        final = final | (data.rd << 7);
        final = final | (data.opcode << 0);
    ...
    case S:
        //Separa imediato
        parte = data.imm & 0b111111100000;
        parte = parte >> 5;

        final = final | (parte << 25);
        final = final | (data.rs2 << 20);
        final = final | (data.rs1 << 15);
        final = final | (data.funct3 << 12);
        
        //Separa imediato
        parte = data.imm & 0b11111; 
    ...
    case B:
        //Separa imediato
        parte = data.imm & 0b1000000000000;
        parte = parte >> 12;
        final = final | (parte << 31);
        
        //Separa imediato
        parte = data.imm & 0b11111100000;
        parte = parte >> 5;
        final = final | (parte << 25);
    ...
    case J:
        //Separa imediato
        parte = data.imm & 0b100000000000000000000;
        parte = parte >> 20;
        final = final | (parte << 31);

        //Separa imediato
        parte = data.imm & 0b11111111110;
        parte = parte >> 1;
        final = final | (parte << 21);
        ...
        //Separa imediato
        parte = data.imm & 0b11111111000000000000;
        parte = parte >> 12;
        final = final | (parte << 12);
