...
main_isr:
    ...
    li t2, 1 # t2 = 1
    sb t1, 0(t0) # ativa a leitura 
    2: #Espera a leitura terminar
        lb t2, 0(t0) # 
        bne t2, zero, 2b # if t1 != zero then 2b
    2:
     
    lw t2, 4(t0) # carrega o tempo atual
    sw t2, 0(t1) # salva o tempo atual
    
    li t2, 100 # t2 = 100
    sw t2, 8(t0) # programa a próxima interrerupção
...
play_note:
    ...
    sh a3, 5(t0) #define a velocidade da nota
    sh a2, 4(t0) #define qual a nota musical
    sh a1, 2(t0) #define o id do instrumento
    sh a0, 0(t0) #define o canal e inicia a reprodução
